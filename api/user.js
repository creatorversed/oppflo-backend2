...
module.exports = async (req, res) => {
  const action = typeof req.query?.action === 'string' ? req.query.action.toLowerCase().trim() : '';
  const isStreakRoute = (req.url || '').includes('/streak') || action === 'streak';
  ...
  if (isStreakRoute && req.method !== 'GET') {
    res.setHeader('Content-Type', 'application/json');
    res.status(405).json({ error: 'Method not allowed' });
    return;
  }

  if (!isStreakRoute && !['GET', 'PATCH'].includes(req.method)) {
    res.setHeader('Content-Type', 'application/json');
    res.status(405).json({ error: 'Method not allowed' });
    return;
  }
