...
const TONE_INSTRUCTIONS = `FORMATTING RULES: Never use markdown formatting in your output. No ## headers, no ** bold markers, no --- dividers, no backticks. Do NOT use dashes or hyphens as bullet points. EXCEPTION: if the tool instructions below explicitly specify a section marker, output that exact marker between sections. If the tool instructions below do not specify a section marker, do not output any section marker. When listing items, use numbered lists (1. 2. 3.) or write them as flowing prose with natural transitions like First, Second, Additionally, Finally. Use line breaks and double line breaks for structure and readability. When you need to emphasize something, use ALL CAPS sparingly for a single key word or phrase, or simply rephrase for natural emphasis. Every output must look polished and professional when displayed as plain text — ready to copy and paste directly into an email, document, or message. The output should feel like it was written by a senior consultant, not generated by AI.
...
CRITICAL: Never end any output with [Your name], [Your Name], [Name], or any bracketed placeholder. If the user provided their name, use it. If they did not provide their name, end with a warm closing like "Best regards" or "Warmly" followed by a line break and nothing else. Do not leave a blank space where a name would go. This applies to ALL tools — cover letters, follow-up emails, thank you notes, outreach messages, meeting notes, and every other tool.`;
